...
from ...functions.SOLWEIGpython import Solweig_2025a_calc_forprocessing_rust as so
...
def solweig_run(configPath, feedback):
    ...
    if configDict["poi_file"] != "":  # usePOI:
        ...
        if standAlone == 0:
            ...
            for idx, row in pois_gdf.iterrows():
                y, x = rowcol(
                    dsm_transf, row["geometry"].centroid.x, row["geometry"].centroid.y
    ...
    if wallScheme == 1:
        ...
        dirwalls_scheme = wa.filter1Goodwin_as_aspect_v3(walls_scheme.copy(), scale, dsm, None, 100.0 / 180.0)
        ...
        voxelTable, dirwalls_scheme = load_walls(
            voxelTable, param, wall_type, dirwalls_scheme, Ta[0], timeStep, alb_grid, landcover, lcgrid, dsm
        )
    ...
    for i in np.arange(0, Ta.__len__()):
        ...
        if configDict["wallscheme"] == 1:  # folderWallScheme: TODO: Fix for standalone
            ...
            if configDict["wallnetcdf"] == "1":  # wallNetCDF:
                ...
                walls_as_netcdf(
                    voxelTable, rows, cols, met_for_xarray, i, dsm, configDict["filepath_dsm"], netcdf_output
                )
        ...
        if configDict["outputkdown"] == "1":
            if standAlone == 0:
                ...
                common.save_raster(
                    configDict["output_dir"] + "/Kdown_" + time_code + ".tif", Kdown, dsm_transf, dsm_crs
                )
        ...
        if configDict["outputldown"] == "1":
            if standAlone == 0:
                ...
                common.save_raster(
                    configDict["output_dir"] + "/Ldown_" + time_code + ".tif", Ldown, dsm_transf, dsm_crs
                )
        if configDict["outputsh"] == "1":
            if standAlone == 0:
                ...
                common.save_raster(
                    configDict["output_dir"] + "/Shadow_" + time_code + ".tif", shadow, dsm_transf, dsm_crs
                )
