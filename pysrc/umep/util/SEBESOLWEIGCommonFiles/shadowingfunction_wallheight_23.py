...
def shadowingfunction_wallheight_23(
    ...
    while (amaxvalue >= dz) and (np.abs(dx) < sizex) and (np.abs(dy) < sizey):
        ...
        dzprev = dz
        vegsh2 = np.add(
            np.add(np.add(fabovea, gabovea, dtype=float), lastfabovea, dtype=float), lastgabovea, dtype=float
        )
        vegsh2[vegsh2 == 4] = 0.0
    ...
    return (
        (vegsh, sh, vbshvegsh, wallsh, wallsun, wallshve, facesh, facesun, shade_on_wall)
        if walls_scheme is not False
